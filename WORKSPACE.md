...
4. **GitHub Release 发布**：
   - Tag：`v2.4.0`（已推送至远程）
   - Release 地址：`https://github.com/byethan/outlookEmailPlus/releases/tag/v2.4.0`
   - 已上传产物：源码 zip + 扩展 zip
   - 发布说明已同步到 Release 页面
...
7. **打 Tag & 创建 Release**：
   - Tag：`v2.3.0` 已 push
   - Release：`https://github.com/byethan/outlookEmailPlus/releases/tag/v2.3.0` ✅
...
- 用户要求先读取并分析 GitHub Issue：`https://github.com/byethan/outlookEmailPlus/issues/52`
- 诉求关键词：
  - 邮件列表变成倒序
  - 获取验证码失败
...
- `gh release create v2.0.0` → https://github.com/byethan/outlookEmailPlus/releases/tag/v2.0.0
...
**失败分析**：该测试需要真实 CF Worker API（https://temp.example.com），本地无网络访问，属于预期内的环境限制，与本次代码变更无关。
...
#### 126. README 与对外 API 文档同步更新：演示地址切换为 localhost:5001
**时间**：2026-04-16
...
4. 发布执行
   ...
   - 发布页：`https://github.com/byethan/outlookEmailPlus/releases/tag/v1.18.0`
...
1. Compose 现场排查
   ...
   - 因此直接执行 `docker compose up` 时，实际使用的是 `ghcr.io/byethan/outlook-email-plus:hotupdate-test`
...
3. 本地构建镜像验证
   - 本地构建镜像：`ghcr.io/byethan/outlook-email-plus:local-main-20260416`
...
1. Issue 现状核对
   - 对象：`https://github.com/byethan/outlookEmailPlus/issues/39`
   - 核对结论：
...
2. 双仓 `v1.17.0` 镜像复核
   - GHCR：`ghcr.io/byethan/outlook-email-plus:v1.17.0`
   - DockerHub：`ghcr.io/byethan/outlook-email-plus:v1.17.0`
   - 两仓 index digest 一致：
...
4. 推送与 Release
   ...
   - GitHub Release 已创建：
     - `https://github.com/byethan/outlookEmailPlus/releases/tag/v1.17.0`
...
1. 需求读取与现状核对
   - 读取并分析：`https://github.com/byethan/outlookEmailPlus/issues/42`
   - 本地核对现状：
...
3. 结果确认
   - Release 页面已更新：
     - `https://github.com/byethan/outlookEmailPlus/releases/tag/v1.16.0`
...
- 日志显示：
  ...
  - `/api/emails/example@outlook.com?...` → `200`
...
**背景**：
- 用户要求读取并分析 Issue #49：`https://github.com/byethan/outlookEmailPlus/issues/49`，判断“批量检测失效邮箱并删除”是否需要实现；
- 随后用户明确选择 **方案 C（混合方案）**：
  - 全量检测主入口纳入失效识别；
  - 保留独立治理入口用于批量处置。
...
**上下文来源（已核对）**：
- GitHub Issue：`https://github.com/byethan/outlookEmailPlus/issues/43`（需求：集成 moemail）
...
docker run -d \
  ...
  ghcr.io/byethan/outlook-email-plus:v1.13.0
...
1. **代码验证**：逐一对比 `hot-update-ai-prompt.md` 中的描述与实际代码
   ...
   - ✅ GitHub 仓库地址 `byethan/outlookEmailPlus`
